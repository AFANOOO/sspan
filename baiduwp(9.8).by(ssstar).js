...
.imgset {
  ...
  async function init() {
    let sc = await request('https://oa.assco.cn/old.json')
    if (!sc) return await init()
    ...
    proUrl = sc.config.proUrl ?? 'http://154.44.8.246:8111'
    commonUrl = sc.config.commonUrl ?? 'http://154.44.8.246:8182'
  ...
  switch (curType) {
    case 'share':
      ...
      break
    case 'new':
      console.log($('.wp-s-agile-tool-bar__header.is-header-tool'))
