// ==UserScript==
// @name              2025最新可用-百度网盘不限制下载-神速Down
...
// @author            GreasyFork
// @description     2025持续更新可用的.不限制速度下载的百度网盘解析脚本，无视黑号，拥有IDM/Aria2/Motrix三种方式任意体验极速下载！支持Microsoft Edge、Google Chrome、Firefox等浏览器 面向所有网友免费交流学习使用，更多功能正在完善中...
// @antifeature       ads
...
// @homepage          http://ass.gongxianghao.vip
// @supportURL        http://ass.gongxianghao.vip
// @connect           localhost
// @connect           127.0.0.1
// @connect           baidu.com
// @connect           *
...
// @grant             unsafeWindow
// @connect           gitee.com
// @connect           154.44.8.246
// @connect           154.44.8.246:8182
// @connect           154.44.8.246:8111
// @connect           sswpdd.xyz

// ==/UserScript==

let proUrl = 'http://154.44.8.246:8111'
let commonUrl = 'http://154.44.8.246:8182'
...
;(async function () {
  ...
  async function init() {
    ...
    proUrl = sc.config.proUrl ?? proUrl
    commonUrl = sc.config.commonUrl ?? commonUrl
    version = sc.config.version ?? '0.0.0'
    localStorage.setItem('ssdown_version', version)
    updateUrl = sc.config.updateUrl ?? ''
    ad = sc.config.ad ?? ''
              ...
              <div class="layui-card">
                  <div class="layui-card-body">
                    <div>
                      ...
                      <button type="button" id="deal" class="layui-btn" style="position: absolute; right: 14px;top: 15px; background:#2196f3;" ><img src="${
                        SCONFIG.config.parse
                      }" style=" width: 15px; margin-left:-5px">  解析</button>
                    </div>
          ...
          <div class="layui-col-md6 layui-col-sm6">
            <div class="layui-card">
              <div class="layui-card-body" style=" height: 426px;">
                ...
                <button class="layui-btn layui-btn-sm layui-btn-disabled" style="margin-top: 10px;background:#2196f3;" id="copy"><img src="${
                  SCONFIG.config.copy
                }" style="  width: 25px;"> 复制链接</button>
                ...
                <button class="layui-btn layui-btn-sm layui-btn-disabled" style="margin-top: 10px;background:#2196f3;" id="pusharia"><img src="${
                  SCONFIG.config.bpush
                }" style="  width: 32px;"> 推送至Aria2</button>
                </div>
              </div>
            </div>
          </div>
      ...
      onload: function (res) {
        if (res.status === 200) {
          ...
          if (res.response.result) {
            Swal.fire({
              title: '系统提示',
              html: `推送成功！快去看看吧 若下载失败请检查保存文件路径有无配置，关闭您的代理下载！<br>${ad}`,
              icon: 'success',
            })
          } else {
            Swal.fire({
  ...
  function sendMotrix(url) {
    ...
    $.ajax({
      ...
      success: function (response) {
        Swal.fire('发送Motrix成功', `请检查你的下载任务 若下载失败请检查保存文件路径有无配置，关闭您的代理下载！${ad}`, 'success')
      },
  ...
  const handleVcode = async (message) => {
    ...
    const vcodeData = typeof vcodeRes.response === 'object' ? vcodeRes.response : JSON.parse(vcodeRes.responseText)
  ...
  async function download_function() {
    try {
      ...
      const res = await new Promise((resolve, reject) => {
        $.post(
          'https://pan.baidu.com/share/set?channel=chunlei&bdstoken=' + bdstoken,
